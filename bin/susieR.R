...
susie_ss_wrapper <- function(df, R, n, L, var_y = 1, prior_weights = NULL, min_abs_corr = 0.0, low_purity_threshold = 0.5) {
  beta <- df$beta
  se <- df$se
...
main <- function(args) {
  ...
  final_lbf <- inner_join(df_hg38, transposed_lbf, by='rsid')
  ...
  variables <- merge(variables, pip, by='rsid')
  ...
  if (!is.null(cs)) {
    ...
    variables_clpp_hg38 <- inner_join(df_hg38, variables_clpp, by='rsid')
