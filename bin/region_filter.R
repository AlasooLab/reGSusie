...
if (nrow(printable_regions) == 0) {
  ...
  fwrite(data.table(), paste0(phenotype_id, '_NULL_variants.txt'), sep='\t', quote=F, row.names=F)
}
...
if (nrow(printable_regions) > 0) {
  for (i in 1:nrow(printable_regions)) {
    ...
    if (nrow(sub_reg) > 50) {     
      ...
      fwrite(as.data.frame(t(sub_reg$rsid)), paste0(phenotype_id, '_', printable_regions$region[i], '_variants.txt'),
